...
unsigned int frequencies[] = {1020,1294,1330,1349,1392,1440,1494};
byte noteCount = sizeof(noteNames);

char score[] = "C C C   CCCC  CCCC  C C C ";
...
void loop() {
  ...
  float sensor3=random(analogRead(IR));
...
// make a HTTP request:
    ...
    while(client.connected()) {
      if(client.available()){
        ...
        //AMBIL DATA JSON
        const size_t capacity = JSON_OBJECT_SIZE(8) + 140; //cari dulu nilainya pakai Arduino Json 5 Asisten
       ...
       if(String(control1_dibaca)=="1" && 
          String(control2_dibaca)=="1" &&
          String(control3_dibaca)=="1"){
